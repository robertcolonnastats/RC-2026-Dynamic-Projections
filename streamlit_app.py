...
Key Updates:
- ADDED DIAGNOSTICS TAB: Shows data loading status and unmapped teams.
- ROBUST TEAM MAPPING: Added full team names and cleaned keys to prevent data loss.
- FIXED SYNTAX: Corrected entry point check.
"""
...
LUCK_REGRESSION_FACTOR = 0.28  # DECREASED
...
# ==============================================================================
# TEAM NORMALIZATION & MAPPING (FIXED SPACES & ADDED FULL NAMES)
# ==============================================================================
TEAM_NORMALIZATION = {
...
# STRICT MAPPING (Uses Normalized Keys - CLEANED OF SPACES)
PECOTA_TEAM_MAP = {
...
def _load_pecota_data():
    ...
    try:
        ...
        if len(hit_df) < mapped_hit_count:
            unmapped = hit_df[~hit_df['team_id'].notna()]['team_clean'].unique() # This logic is slightly off, but we capture unmapped below
            # Actually, let's capture unmapped before dropping
            pass 
            
        # Better way to capture unmapped:
        hit_df_temp = hit_df.copy() # Before dropna
        hit_df_temp["team_id"] = hit_df_temp['team_clean'].map(PECOTA_TEAM_MAP)
        diag["unmapped_hit"] = hit_df_temp[hit_df_temp['team_id'].isna()]['team_clean'].unique().tolist()[:5]
        
        pit_df_temp = pit_df.copy()
        pit_df_temp["team_id"] = pit_df_temp['team_clean'].map(PECOTA_TEAM_MAP)
        diag["unmapped_pit"] = pit_df_temp[pit_df_temp['team_id'].isna()]['team_clean'].unique().tolist()[:5]
        
        # Now drop na for actual use
...
def compute_buyer_seller(df):
    ...
    # STRICT: USE DEADLINE RAMP FOR TIMING
...
def render_diagnostics_tab():
    ...
    st.markdown("Use this tab to verify that your Excel data is being loaded and mapped correctly.")
    
    diag = st.session_state.get('diagnostics', {})
    
    if not diag:
        st.warning("No diagnostics data available. Please reload the app.")
        return
        
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("### File Status")
        st.write(f"**Hitting File:** {'✅ Found' if diag.get('hit_file_found') else '❌ Missing'}")
        st.write(f"**Pitching File:** {'✅ Found' if diag.get('pit_file_found') else '❌ Missing'}")
        st.write(f"**Hitting Rows Loaded:** {diag.get('hit_rows', 0)}")
        st.write(f"**Pitching Rows Loaded:** {diag.get('pit_rows', 0)}")
    ...
    with c2:
        ...
        if diag.get('error'):
            ...
            st.success("✅ No critical errors during loading.")
            
        unmapped_hit = diag.get('unmapped_hit', [])
        unmapped_pit = diag.get('unmapped_pit', [])
        
        if unmapped_hit:
            st.warning(f"**Unmapped Hitting Teams (Sample):** {unmapped_hit}")
            st.info("If you see full team names here (e.g., 'LOS ANGELES DODGERS'), the mapping is working. If you see abbreviations that don't match (e.g., 'LAD '), check your Excel file for extra spaces.")
        else:
            st.success("✅ All hitting teams mapped successfully.")
            
        if unmapped_pit:
            st.warning(f"**Unmapped Pitching Teams (Sample):** {unmapped_pit}")
        else:
            st.success("✅ All pitching teams mapped successfully.")
...
def main():
    ...
    with tab5: render_diagnostics_tab()
