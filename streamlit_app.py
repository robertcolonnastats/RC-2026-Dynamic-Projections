...
Updates:
- FIXED: Auto-calculates OPS from OBP/SLG if missing in Excel files.
- FIXED: Normalizes Excel column names (strips whitespace/lowercases).
- CONSTANTS: Balanced for Luck (0.45), Statcast (0.45), Pythag (120).
- SIMULATION: Fixed 'high<=0' numpy crash with safe helper function.
"""
...
Fixes:
- Auto-calculates OPS if missing in Excel (Option 2).
- Cleans Excel column names (lowercase/strips whitespace).
- Fixed numpy random crash with safe_randint helper.
"""
import os, json, warnings, sys
import requests, numpy as np, pandas as pd
import plotly.graph_objects as go
...
CACHE_VERSION            = "v20-regression-balance-hotfix"
...
# Balanced constants to pull down lucky teams (like BAL) and boost talent (like LAD)
...
# Stronger regression for Pythagorean win % (trusts baseline talent more)
PYTHAG_REGRESSION_PA     = 120
# More weight to PECOTA talent early in the season
PROJ_WEIGHT_MAX          = 0.75
PROJ_WEIGHT_MIN          = 0.45

# Better buyer/seller separation
...
# Stronger regression of luck (pulls down lucky teams like NYY/BAL)
...
def _load_pecota_data():
    ...
    try:
        ...
        if os.path.exists(hit_file) and os.path.exists(pit_file):
            ...
            # OPTION 2 FIX: Auto-calculate OPS if missing
...
def fetch_team_projections(standings_df, roster_map):
    ...
    for tid in all_ids:
        ...
        # GS Percentage Logic (Checks for columns first to prevent errors)
...
Teams underperforming due to injuries are systematically undervalued — their odds don't reflect the roster they'll have in August.""")
    ...
    with st.expander("🗓️ Deadline Ramp"): st.markdown(f"""Ramp: **{DEADLINE_RAMP_START} → {TRADE_DEADLINE}** · Today: **{get_deadline_ramp_factor():.1%}**\n\n`ramped_adj = base_adj × ramp_factor`\n\nBefore ramp start, no adjustment applied to projected records.""")
    with st.expander("📉 Luck Regression & SoS"): st.markdown(f"""**Luck regression:** `-(luck_wins × {LUCK_REGRESSION_FACTOR}) / games_remaining`\n\n**SoS:** `(0.500 − avg_opp_win_pct) × {SOS_SENSITIVITY} × min(GP/81, 1.0)`""")
    with st.expander("🎲 Monte Carlo Simulation"): st.markdown(f"""{N_SIMULATIONS:,} sims · Log5 win probability · Zero-sum (total wins = games played)\n\nTwo parallel runs: post-deadline (with adj) and pre-deadline (without adj) for Deadline Impact comparison.""")
...
def load_all_data():
    ...
    st.markdown("### ⚾ Loading fresh data..."); pb = st.progress(0); tx = st.empty()
