...
CACHE_VERSION = "v44-calibrated-fine-tune"
...
def _load_pecota_data():
    ...
    # Note: In a real deployment, these would be loaded from files. 
    # For this script, we assume the embedded data or file loading logic handles it.
    # To keep the script self-contained for the user, we'll use a placeholder for file loading
    # but ensure the logic matches the user's request.
    ...
    try:
        ...
        if os.path.exists(hit_file) and os.path.exists(pit_file):
            ...
            # Hitters: Cap contribution at 550 PA
            if 'pa' in hit_df.columns:
                hit_df['adj_warp'] = hit_df['warp'] * (hit_df['pa'] / 550).clip(upper=1)
            else:
                hit_df['adj_warp'] = hit_df['warp']
                
            # Pitchers: Cap contribution at 180 IP
            if 'ip' in pit_df.columns:
                pit_df['adj_warp'] = pit_df['warp'] * (pit_df['ip'] / 180).clip(upper=1)
            else:
                pit_df['adj_warp'] = pit_df['warp']

            # Roster Capping: Top 9 Hitters, Top 4 SP, Top 5 RP
            if 'pa' in hit_df.columns:
    ...
    for tid in all_ids:
        try:
            ...
            if baseline_wpct < 0.500:
                ...
                baseline_wpct -= downside_gap * 0.20
...
def render_methodology_tab():
    ...
    with st.expander("🔮 Projection Engine"):
        ...
        - **Base Wins**: 44.0 (Replacement Level)
        - **Weighted WARP**: 
          - Hitters: `WARP * min(1, PA / 550)`
          - Pitchers: `WARP * min(1, IP / 180)`
        - **Formula**: `Baseline Wins = 44.0 + Sum(Weighted WARP)`
        - **Downside Penalty**: Teams below .500 WPCT receive an additional penalty: `WPCT -= (0.500 - WPCT) * 0.20`
        ...
        - **Statcast Blend**: Influence of {STATCAST_INFLUENCE}
        ...
        - **Fixed Regression**: {PYTHAG_REGRESSION_PA} games
    ...
    with st.expander("📈 Buyer/Seller Classification"):
        ...
        st.markdown(f"""
        **Score Formula:** `Score = WC GB + RD Modifier + Luck Modifier`
    ...
    with st.expander("🗓️ Deadline Ramp"):
        ...
        **Today's Factor:** {get_deadline_ramp_factor():.1%}
        
        **Formula:** `ramped_adj = base_adj × ramp_factor`
        
        Before ramp start, no adjustment is applied to projected records.
        """)
    ...
    with st.expander("🎲 Monte Carlo Simulation"):
        ...
        - **Zero-Sum Constraint**
        - **Two Parallel Runs**: 
          1. Post-deadline (with adjustments)
          2. Pre-deadline (without adjustments)
