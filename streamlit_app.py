...
✅ PECOTA data embedded — no external files required
"""
import os, json, warnings, sys, base64, io
...
def fetch_standings():
    ...
    for rec in resp.json().get("records",[]):
        for tr in rec.get("teamRecords",[]):
            ...
            w,l = tr.get("wins",0),tr.get("losses",0); gp = w+l
            wp = w/gp if gp>0 else 0.0
            try: gb = float(tr.get("gamesBack","0"))
            except: gb = 0.0
            rs,ra = tr.get("runsScored",0) or 0,tr.get("runsAllowed",0) or 0
            rows.append({"team_id":tid,"name":nm,"abbr":ab,"division":div,"league":lg,"wins":w,"losses":l,
...
# Global cache for PECOTA data (embedded)
_PECOTA_HIT_DF = None
_PECOTA_PIT_DF = None

def _load_pecota_data():
    """Load PECOTA data from embedded JSON strings"""
...
def fetch_team_projections(standings_df, roster_map):
    ...
    for tid in all_ids:
        ...
        if not lineup.empty:
            rw = [ROSTER_WEIGHT_ACTIVE if r["mlbid"] in active_ids else
                  (ROSTER_WEIGHT_IL if r["mlbid"] in il_ids else ROSTER_WEIGHT_OTHER)
                  for _,r in lineup.iterrows()]
            pecota_ops = float(np.average(lineup["ops"].fillna(LEAGUE_AVG_OPS),weights=rw))
        ...
        def staff_era(df,role):
            ...
            cip = df["ip"].clip(upper=cap)
            blend = (df["fip"].fillna(LEAGUE_AVG_FIP)*0.7+df["era"].fillna(LEAGUE_AVG_ERA)*0.3).clip(2,7.5)
            return float(np.average(blend,weights=cip)) if cip.sum()>0 else LEAGUE_AVG_ERA
        sp_base = float(np.clip(staff_era(sp,"SP"),2.80,5.50))
        rp_base = float(np.clip(staff_era(rp,"RP"),3.00,5.50))
        ...
        il_warp = float(ph[(ph["team_id"]==tid)&(ph["mlbid"].isin(il_ids))]["warp"].clip(lower=0).sum())
        rows.append({"team_id":tid,"proj_runs_per_game":round(proj_rpg,3),"proj_ra_per_game":round(proj_rapg,3),
                     "proj_win_pct":round(proj_wp,4),"il_warp":round(il_warp,2),"proj_source":"PECOTA+Statcast"})
...
def _sim_once(mdf,sch,wp_col,rng):
    ...
    div_map=mdf.set_index("team_id")["division"].to_dict(); lg_map=mdf.set_index("team_id")["league"].to_dict()
...
| **PECOTA 2026** | Static | Talent baseline — 50th percentile depth chart (✅ embedded) |""")
    with st.expander("🔮 Projection Engine"):
        st.markdown(f"""**PECOTA baseline:** Top 9 by projected PA. Active: {ROSTER_WEIGHT_ACTIVE:.0f}× weight, IL: {ROSTER_WEIGHT_IL:.0f}× weight, other: {ROSTER_WEIGHT_OTHER:.0f}× weight. SP cap {IP_FULL_WEIGHT_SP} IP, RP cap {IP_FULL_WEIGHT_RP} IP. No aging curve — PECOTA already prices age.
...
def load_all_data():
    ...
    try:
        ...
        for _,row in std.iterrows():
            ...
            wp=pythag(row.get("runs_scored",0)/gp,row.get("runs_allowed",0)/gp) if gp>0 else 0.500
            rows.append({"team_id":int(row["team_id"]),"proj_win_pct":round(wp,4),"proj_runs_per_game":LEAGUE_AVG_RPG,
...
def main():
    # ✅ No file checks needed — PECOTA data is embedded!
