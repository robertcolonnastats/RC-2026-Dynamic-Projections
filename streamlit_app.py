...
Key Updates:
- Fixed 'team' KeyError by normalizing Excel column names (strip + lowercase).
- Added debugging output to show actual columns if 'team' is still missing.
- Restored full embedded PECOTA data as a fallback.
- GS% Logic: Pitchers classified as SP only if GS/G >= 0.50.
- Constants: Luck (0.45), Statcast (0.45), Pythag (120), etc.
"""
...
def _load_pecota_data():
    ...
    try:
        ...
        if os.path.exists(hit_file) and os.path.exists(pit_file):
            ...
            # FIX: Normalize column names to handle whitespace/case issues
            ...
            if 'team' not in hit_df.columns:
                st.error(f"Missing 'team' column in hitting file. Found columns: {hit_df.columns.tolist()}")
                st.stop()
            if 'team' not in pit_df.columns:
                st.error(f"Missing 'team' column in pitching file. Found columns: {pit_df.columns.tolist()}")
                st.stop()
            ...
            # Drop rows where team mapping failed (e.g. unknown team names)
        ...
        st.error(f"Error loading PECOTA data: {e}")
        st.stop()
