...
Key Updates:
- FIXED: Added "Extra Downside Penalty" logic to crush bad teams.
- STRENGTHENED: Increased penalty sensitivity to 0.60 (60%) for visible impact.
- STRUCTURE: Integrated Penalty logic directly into WARP Baseline calculation.
"""
...
# UPDATED: Reduced Statcast noise impact to prevent over-smoothing
...
# UPDATED: Fixed regression at 45 games (reduced from dynamic curve)
...
def fetch_team_projections(standings_df, roster_map):
    ...
    for tid in all_ids:
        try:
            ...
            # Calculate Baseline Wins: 45 + Weighted WARP
            baseline_wins = 45.0 + team_warp_sum
            baseline_wpct = baseline_wins / 162.0

            # ==============================================================================
            # EXTRA DOWNSIDE PENALTY FOR WEAK TEAMS (AGGRESSIVE)
            # ==============================================================================
            # PURPOSE: Allow terrible teams to collapse more realistically.
            # Sensitivity increased to 0.60 for visible impact.
            DOWNSIDE_SENSITIVITY = 0.60 

            if baseline_wpct < 0.500:
                downside_gap = 0.500 - baseline_wpct
                # New WPCT = Base - (Gap * Sensitivity)
                baseline_wpct -= downside_gap * DOWNSIDE_SENSITIVITY
