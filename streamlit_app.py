...
Updates:
- Rebalanced projection constants (Statcast 0.45, Pythag 120, Luck Reg 0.45)
- Fixed high<=0 numpy crash in simulation loop
- Normalized Excel column names for robust parsing
- Restored full embedded PECOTA fallback data
"""
...
# ==============================================================================
# TEMPORARY CACHE CLEARING (Uncomment below for ONE run, then comment out)
# ==============================================================================
...
# ==============================================================================
# EMBEDDED PECOTA DATA (Full Fallback)
# ==============================================================================
...
# Updated constants per your request
...
def _load_pecota_data():
    ...
    try:
        ...
        if os.path.exists(hit_file) and os.path.exists(pit_file):
            ...
            if 'team' not in hit_df.columns: st.error(f"Missing 'team' column in hitting file. Found columns: {hit_df.columns.tolist()}"); st.stop()
            if 'team' not in pit_df.columns: st.error(f"Missing 'team' column in pitching file. Found columns: {pit_df.columns.tolist()}"); st.stop()
