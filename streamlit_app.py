...
- Age 35+ power decline is steeper than co ntact decline
...
- Young starters frequently i mprove command from age 22-27
...
Returns dict: {player_id: {hitting: {...}, pitc hing: {...}}}
...
acti ve_pitchers: list of {name, era, fip, ip, role}
...
try:
    ...
    prior _stats  = _fetch_team_player_stats(team_id, SEASON_YEAR - 1)
    ...
    for entry in roster:
        ...
        if pos_type ==  "Pitcher ":
            ...
            career_era  = safe_ era(career_pit, LEAGUE_AVG_ERA)
            ...
            era = (c areer_era * career_weight + prior_era * prior_weight +
            ...
            career _gs   = int(career_pit.get( "gamesStarted ", 0) or 0)
            ...
            prior_weight = 0.30
            career_weight = 1.0 - cur_weight - prior_weight
            ops = (career_ops * career_weight + prior_op s * prior_weight + 
            ...
            if age is not None:
                ...
                ops = float(np.c lip(ops * aging_mult, 0.400, 1.200))
...
pro j_rapg = float(np.clip(proj_rapg, 2.5, 7.5))
...
Returns (team_proj_df, play er_detail_dict)
 " " "
...
try:
    ...
    # Also try FanGraphs DC quickl y (may work from Streamlit Cloud)
    try:
...
if standings_df is not None and not standings_df.empty:
    rows = []
    for _, row in standings_d f.iterrows():
        tid  = row[ "team_id "]
...
- Determine how many of those days fall before vs after th e deadline
...
The adjustment feeds into buyer/seller score:
  - Stars ou t pre-deadline → team looks worse than they are → pull toward buyer
...
for p in il_players:
    ...
    post_dl_win_impact = war_per_game * days_out_post_dl * GAMES _PER_DAY_IMPACT
    ...
    # Post-deadline injuries: doesn't h elp pre-deadline but team truly is weaker
...
#   league average. DO NOT blend with Pythagorean again — t hat double-counts
...
# Late season: Pythagorean ge ts more weight (larger sample)
...
def run_simulation(master_df: pd.DataFrame, schedule_df: pd.DataFrame) -> dict:
    ...
    base_probs = np.array([log5(base_wp.get(h, 0.5), base_wp.get(a, 0.5)) for h, a in zi p(home_ids, away_ids)])
    home_idx   = np.array([tid_idx[h] for h in home_ids])
    away_idx   = np.array([tid_idx[a] for a in away_ids])
    n_games    = len(home_ids)

    # ⚡ VECTORIZED REPLACEMENT (replaces slow Python for-loop)
    def sim_batch(probs):
        finals = np.tile(init_wins, (N_SIMULATIONS, 1)).astype(float)
        if n_games == 0:
            return finals
        rand = rng.random((N_SIMULATIONS, n_games))
        hw = (rand < probs[np.newaxis, :]).astype(float)
        # Fast C-level accumulation without Python loop overhead
        ...
        for sim in range(N_SIMULATIONS):
            ...
            for league in [ "AL ",  "NL "]:
                ...
                if non_div:
                    ...
                    for r in top3: po_coun t[non_div[r]] += 1
            ...
            rem = list(playoff)
             while len(rem)  > 1:
                ...
                for i in range(0, len(rem) - 1, 2):
                    ...
                    nx t.append(rem[i] if rng.random()  < p else rem[i+1])
...
fig = go.Figure()
fig.add_tra ce(go.Scatter(x=x, y=y, fill= "tozeroy ", mode= "lines ",
...
Run differential modifier — positive run diff pulls toward buyer (may be unlucky);
negative pushes toward seller (may be lucky)
Pythagorean luck modifier — teams outperforming their Pythagorean win% are
...
| ---|---|---|
| Hard Seller
|{int(HARD_SELLER_GB)}+ GB
|{ADJ_HARD_SELLER:.0%}
|
| Soft Seller
|{int(SOFT_SELLER_GB)}-{int(HARD_SELLER_GB)} GB
|{ADJ_SOFT_SELLER:.0%}
|
| Neutral
|Within {int(NEUTRAL_BAND)} GB
|0%
|
| Soft Buyer
|In WC picture
...
try:
    with _scf.ThreadPoolExecutor(max_workers=1) as _sex:
        schedule_df = _sex.submit(fetch_schedule).result(timeo ut=60)
except Exception:
...
try:
    with _pf.ThreadPoolExecutor(max_workers=1) as _px:
        statcast_df, player_detail = _px.submit(
            fetc h_team_projections, standings_df
        ).result(timeout=180)
...
try:
    if schedule_df is not None and not schedule_df.empty:
        opps      = compute_rema ining_opponents(schedule_df)
...
# Logo — looks for rc_logo.png in rep o root
import os
...
if master_df is not None and not master_df.empty:
    sa mple_proj = list(sim_results.get( "proj_wins ", {}).values())
