...
Key Updates:
Restored full logic (Projections, Sim, UI) - No truncation
Updated weights to expand projection range (LAD ~101, NYY ~95)
Robust team name mapping (Handles Full Names & Abbreviations)
Added debugging info for Excel data loading
"""
...
# ==============================================================================
# CONSTANTS - UPDATED WEIGHTS TO FIX COMPRESSION
# ==============================================================================
SEASON_YEAR = 2026
...
CACHE_VERSION = "v37-expanded-range-fix"

PA_FULL_WEIGHT = 400
IP_FULL_WEIGHT_SP = 150
IP_FULL_WEIGHT_RP = 40
PRIOR_PECOTA_WEIGHT = 0.58  # INCREASED to trust PECOTA more
PRIOR_HIST_2025_WEIGHT = 0.35
PRIOR_HIST_2024_WEIGHT = 0.20

STATCAST_INFLUENCE = 0.22  # DECREASED to reduce noise
ROSTER_WEIGHT_ACTIVE = 650.0
ROSTER_WEIGHT_IL = 8.0
ROSTER_WEIGHT_OTHER = 280.0
TYPICAL_TEAM_WARP = 35.0
MAX_IL_FRAC = 0.50
PYTHAG_REGRESSION_PA = 190   # INCREASED to keep projections from regressing to .500 too fast
PROJ_WEIGHT_MAX = 0.90      # INCREASED to trust talent projection more
...
LUCK_REGRESSION_FACTOR = 0.28  # DECREASED to stop punishing lucky teams
...
ADJ_HARD_BUYER = +0.07
ADJ_SCALE = 0.015
SOS_SENSITIVITY = 0.09  # DECREASED to reduce hard schedule penalty
...
def _load_pecota_data():
    ...
    try:
        if os.path.exists(hit_file) and os.path.exists(pit_file):
            ...
            pit_df['team_norm'] = pit_df['team'].apply(normalize_team)
            
            # Map to team IDs
            hit_df["team_id"] = hit_df['team_norm'].map(PECOTA_TEAM_MAP)
            pit_df["team_id"] = pit_df['team_norm'].map(PECOTA_TEAM_MAP)
            ...
            all_team_ids = set(TEAM_INFO.keys())
            missing_teams = [t for t in all_team_ids if t not in loaded_teams]
            ...
            if missing_teams:
                ...
                st.warning(f"⚠️ Missing data for {len(missing_teams)} teams: {', '.join(missing_names)}")
        else:
