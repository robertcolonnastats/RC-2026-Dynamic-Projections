...
Key Updates:
Robust Team Name Mapping (Handles Full Names & Abbreviations)
Aggressive Weights (Fixes Compression Issue)
...
CACHE_VERSION            = "v36-robust-mapping-fix"
PA_FULL_WEIGHT           = 400
...
hit_df['team_clean'] = hit_df['team_clean'].replace({'CWS': 'CHW', 'CHW': 'CHW'})
hit_df["team_id"] = hit_df['team_clean'].map(PECOTA_TEAM_MAP)
else:
st.error("Team column missing in hitting file")
st.stop()
if 'team' in pit_df.columns:
pit_df['team_clean'] = pit_df['team'].astype(str).str.strip().str.upper()
pit_df['team_clean'] = pit_df['team_clean'].replace({'CWS': 'CHW', 'CHW': 'CHW'})
pit_df["team_id"] = pit_df['team_clean'].map(PECOTA_TEAM_MAP)
else:
st.error("Team column missing in pitching file")
st.stop()
...
if len(loaded_teams) < 30:
missing = [TEAM_INFO[tid][0] for tid in TEAM_INFO if tid not in loaded_teams]
st.warning(f"⚠️ Missing teams: {', '.join(missing)}")
else:
